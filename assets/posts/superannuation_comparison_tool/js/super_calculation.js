...
function updateCalc() {
  ...
  if (checkValidValue(years, 0) == false) {
    return;
  }
  ...
  for (var n = 1; n < 4; n++) {
    ...
    if (checkValidValue(current_balance, 2) == false) {
      return;
    }
    ...
    var total_annual_contribution = contribution_amount * contributions_per_year;
      // check if greater than 0 and up to 2 decimal places
    if (checkValidValue(total_annual_contribution, 2) == false) {
      return;
    }
    ...
    var annual_flat_fee = parseFloat($('#flatfeeamount-' + n).val()) * feeFreq;
      // check if greater than 0 and up to 2 decimal places
    if (checkValidValue(annual_flat_fee, 2) == false) {
      return;
    }
    ...
    var annual_percentage_fee = parseFloat($('#percfee-' + n).val());
      // check if greater than 0 and up to 2 decimal places
    if (checkValidValue(annual_percentage_fee, 2) == false) {
      return;
    }
...
function checkValidValue(input_variable, decimal_places) {
  if (input_variable < 0 || !Number.isInteger(Math.pow(10, decimal_places) * input_variable)) {
    return false;
  }
}
